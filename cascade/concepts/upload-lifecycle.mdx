...
After registration, a Cascade upload moves through these states. For the full `ActionState` enum see [Actions](/intelligence-layer/verifiable-execution/actions).
...
| `EXPIRED` | No SuperNode finalized before `expirationTime` |

The storage fee is escrowed at registration. The chain refunds it if the action expires before finalization. The current fee is a 10000 `ulume` base plus 10 `ulume` per KB. A 1 MB file costs about 0.02 LUME. Fee parameters can change through governance.
