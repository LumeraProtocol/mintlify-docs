...
<Accordion title="chain-config.ts for Keplr">
...
// Expiration, 25 hours from now as a Unix timestamp
...
`result.action_id` is the permanent on-chain reference to your file. Save it. You need it for every download.
...
The state starts at `PENDING` after registration. It moves to `PROCESSING` while SuperNodes encode and distribute the file, then ends at `DONE`. An upload still unfinalized at its `expirationTime` ends at `EXPIRED` and the escrowed fee is refunded.
