...
Keplr asks the user to approve every signature request. A click on Reject surfaces as an error.
...
const result = await client.Cascade.uploader.uploadFile(file, {
  ...
  isPublic: true,
  expirationTime: String(Math.floor(Date.now() / 1000) + 25 * 60 * 60),
  taskOptions: {
...
The storage fee is escrowed on chain when you register an action. The chain refunds it to your wallet automatically when the action expires because no SuperNode finalized it before its `expirationTime`. A rejected finalization does not refund on its own; the action stays open until it is finalized or expires.
