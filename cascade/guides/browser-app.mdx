...
sidebarTitle: "Browser Application"
description: "Build a Vite and TypeScript single page app that stores files on Cascade through Keplr."
---

In this guide you build a Vite and TypeScript single page app that connects to the Keplr wallet, uploads files to Cascade, and retrieves them again by action ID. Cascade is the permanent storage service of Lumera Protocol. You pay once at upload time and the file stays available forever.

Every upload returns an action ID. This is the permanent on-chain reference you later use to download the file.

The app talks to Cascade straight from the browser. The SDK handles the SN-API transfer, so you do not need a backend server for storage.

<img
  className="block mx-auto dark:hidden"
  alt="Browser-first Cascade architecture"
  src="/images/diagrams/node-architecture-light.svg"
/>
<img
  className="mx-auto hidden dark:block"
  alt="Browser-first Cascade architecture"
  src="/images/diagrams/node-architecture-dark.svg"
/>

## What you need

- [Node.js](https://nodejs.org/) 18 or later
- The [Keplr](https://www.keplr.app/) browser extension
- Testnet LUME for fees. Request tokens from the [faucet](/faucet).
...
<Steps>
  ...
  <Step title="Add the chain configuration">
  ...
  <Step title="Connect the Keplr wallet">
  ...
  <Step title="Create the Cascade client">
...
1. Use `getKeplrSigner` from the SDK. It wraps the Keplr extension signer with the ADR-036 `signArbitrary` support that Cascade requires.
...
<CardGroup cols={2}>
  ...
  <Card title="Node.js guide" icon="server" href="/cascade/guides/nodejs">
