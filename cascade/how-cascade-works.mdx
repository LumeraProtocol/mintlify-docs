...
Your app (SDK)
   |
   |  1. MsgRequestAction registers the action, the fee is escrowed
   v
Lumera chain
   |
   |  2. action_registered event returns the action_id
   v
Your app (SDK)
   |
   |  3. Multipart POST sends the file to SN-API
   v
SuperNode
   |
   |  4. Verify, encode with RaptorQ, distribute symbols
   v
SuperNode mesh
   |
   |  5. MsgFinalizeAction marks the action DONE
   v
Lumera chain
```
