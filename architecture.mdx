...
<Steps>
  ...
  <Step title="Finalize">
    When the work completes and results are committed, the state becomes `DONE`. An action still unfinalized at its `expirationTime` becomes `EXPIRED` and the chain refunds the escrowed fee.
  </Step>
  <Step title="Approve">
...
Every protocol Action pays a one-time service fee in `ulume` (1 LUME = 1,000,000 ulume). The fee is a pure function of data size, so you can estimate it before submitting. File size is rounded up to the next whole kilobyte.
...
<Tabs>
  <Tab title="Mainnet">
    | Parameter | Value | In LUME |
    | --- | --- | --- |
    | `base_action_fee` | `30000` ulume | 0.03 LUME |
    | `fee_per_kbyte` | `10` ulume | 0.00001 LUME per KB |

    Every successful storage adds `30000 + 10 × sizeKB` ulume to what you have spent.

    | File size | Storage fee (ulume) | In LUME |
    | --- | --- | --- |
    | 1 KB | 30,010 | 0.03001 |
    | 1 MB (1,024 KB) | 40,240 | 0.04024 |
    | 10 MB (10,240 KB) | 132,400 | 0.13240 |
    | 100 MB (102,400 KB) | 1,054,000 | 1.05400 |
    | 1 GB (1,048,576 KB) | 10,515,760 | 10.51576 |
  </Tab>
  <Tab title="Testnet">
    | Parameter | Value | In LUME |
    | --- | --- | --- |
    ...
    Every successful storage adds `10000 + 10 × sizeKB` ulume to what you have spent.
...
Governance can change these parameters, so query the exact amount from the chain instead of hardcoding it. The SDK expects the size in KB, rounded up, and returns the fee in `ulume`.

```ts
const fileSizeKb = Math.ceil(file.length / 1024);
const { amount } = await client.Blockchain.Action.getActionFee(fileSizeKb);
// amount is a string in ulume. The SDK sets this as the action price.
```

The fee is locked in escrow at registration and released when the Action settles, following [the action lifecycle](#the-action-lifecycle). On mainnet, 98% of each fee pays the SuperNodes that served the Action and 2% goes to the foundation. If the Action expires before finalization, the chain refunds the full fee. You pay once, with no recurring charges.

The service fee is separate from the gas fee charged on every transaction. The minimum gas price is `0.025ulume`. See [Networks](/networks) for chain parameters and [Protocol revenue](/protocol-revenue) for how fees flow through the network.
