...
Resolves to the completed upload `Task`. It does **not** return the action ID. `uploadFile` registers the action internally and then discards the reference, so use the lower level steps below whenever you need to keep it.

### Lower level upload steps

Use these when you need control over individual phases, or when you need the action ID.
