...
<img className="block dark:hidden mx-auto" src="/images/diagrams/encrypted-storage-light.svg" alt="Encrypted Storage Architecture" />
<img className="hidden dark:block mx-auto" src="/images/diagrams/encrypted-storage-dark.svg" alt="Encrypted Storage Architecture" />

```text
wallet signs a fixed message (ADR-036)
        |
        v  BLAKE2b hash of the signature
wallet key (256 bit, re-derived on demand, never stored)
        |
        v  encrypts
document key (random, generated once per file)
        |
        v  encrypts
file bytes
        |
        v
manifest JSON (ciphertext, nonces, wrapped document key)
        |
        v
upload to Cascade (public but unreadable without the keys)
```
