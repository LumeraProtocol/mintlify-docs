...
<img
  ...
  alt="An action starts PENDING with its fee escrowed, moves to PROCESSING while SuperNodes execute, and ends DONE with the fee released. From either PENDING or PROCESSING it can instead end FAILED, which returns the escrow."
...
| `FAILED` | Finalization failed. The escrow returns to the creator |
