...
description: "Use the lumerad command line tool to query chain state, submit transactions, manage keys, and work with Lumera modules."
---
...
<Tabs>
  ...
  <Tab title="Claims">
    ```shell
    # Show the claim record for a legacy address
    lumerad query claim claim-record <old-address>

    # List claimed records for a vested term (0 if not vested)
...
<Tabs>
  ...
  <Tab title="Claims">
    ```shell
    # Claim LUME to a new address, signed with the legacy key
    lumerad tx claim claim <old-address> <new-address> <pub-key> <signature> \
      --from <key-name> \
      --chain-id lumera-mainnet-1

    # Same claim into a vesting tier
    lumerad tx claim delayed-claim <old-address> <new-address> <pub-key> <signature> <tier> \
      --from <key-name> \
      --chain-id lumera-mainnet-1
    ```
  </Tab>
...
<AccordionGroup>
  ...
  <Accordion title="Query an action and verify it is finalized">
    ```shell
    lumerad query action action <action-id> --output json | jq '.action.state'
    ```

    A finalized Cascade action has `state: "ACTION_STATE_DONE"`.
  </Accordion>
