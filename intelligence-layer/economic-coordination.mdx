...
If the Action completes and is confirmed, the escrow releases to the SuperNodes that served it. If it expires before finalization, the escrow returns to the requester. Neither outcome depends on either party's cooperation after the fact, because the condition is evaluated against the on-chain record.
