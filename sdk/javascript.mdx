...
Browser wallets must know the Lumera chain before they can sign for it. Register it once with `experimentalSuggestChain` using this config. The config uses coin type 60 with the Ethereum key features because Lumera derives keys the Ethereum way since the `v1.20.0` upgrade.
