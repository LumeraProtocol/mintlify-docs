...
An action still unfinalized at its `expirationTime` becomes `EXPIRED` and the escrowed fee is refunded. The [architecture page](/architecture) walks the full action state machine that Sense shares with Cascade.
