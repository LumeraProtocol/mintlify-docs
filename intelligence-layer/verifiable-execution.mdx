...
**Settlement gated on acknowledgment.** Payment releases when completion is confirmed against the on-chain record. Actions that expire before finalization refund the escrow. The requester is not paying for a promise, and the operator is not serving on credit.
