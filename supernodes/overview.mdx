...
```
 Validator host                          SuperNode host
 ┌────────────────────┐                  ┌────────────────────┐
 │ lumerad            │                  │ supernode          │
 │ consensus P2P      │                  │ managed by         │
 │ port 26656         │                  │ sn-manager         │
 └─────────┬──────────┘                  └─────────┬──────────┘
           │ produces blocks                       │ gRPC, reads state and
           │                                       │ broadcasts messages
           ▼                                       ▼
 ┌──────────────────────────────────────────────────────────────┐
 │                        Lumera chain                          │
 │  register-supernode binds VALOPER + SN_ENDPOINT + SN_ACCOUNT │
 │  SN_ACCOUNT delegates stake to VALOPER (LEP-3)               │
 └──────────────────────────────────────────────────────────────┘

 SuperNode ◄── Kademlia DHT, port 4445 ──► other SuperNodes
```
