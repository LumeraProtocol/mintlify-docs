---
title: "CascadeService gRPC reference"
...
<Note>
  Two layers reach Cascade. Applications go through [SN-API](/api/sn-api), the HTTP gateway in front of the SuperNode network, which the [JavaScript](/sdk/javascript) and [Rust](/sdk/rust) SDKs use. `CascadeService` is the direct path to a SuperNode on port 4444, used by the [Go SDK](/sdk/go) and operator tooling. Use SN-API for app-facing upload and download.
</Note>
...
<ResponseField name="event" type="DownloadEvent">
  ...
  <Expandable title="DownloadEvent fields">
    <ResponseField name="event.event_type" type="SupernodeEventType">
      The event milestone. Download emits `ACTION_RETRIEVED`, `METADATA_DECODED`, `NETWORK_RETRIEVE_STARTED`, `ARTEFACTS_DOWNLOADED`, and `DECODE_COMPLETED`, then `SERVE_READY` immediately before the first `chunk`.
    </ResponseField>
...
| `14` | `FINALIZE_SIMULATION_FAILED` | Register |
| `15` | `NETWORK_RETRIEVE_STARTED` | Download |
| `16` | `DECODE_COMPLETED` | Download |
| `17` | `SERVE_READY` | Download |

---

## HTTP alternative

`CascadeService` has no HTTP gateway. The SuperNode's own gateway on port 8002 exposes `SupernodeService` only, documented in [`docs/gateway.md`](https://github.com/LumeraProtocol/supernode/blob/master/docs/gateway.md). To move Cascade files over HTTP, use [SN-API](/api/sn-api).
